...
class ProfileService {
  ...
  async updateProfile(profileId, profileData) {
    try {
      ...
      if (profileData.displayName) {
        profileData.slug = this.generateSlug(profileData.displayName);
      }
  ...
  async registerProfileView(profileId, viewerId = null, metadata = {}) {
    try {
      const { 
        ipAddress, sessionId, deviceType, deviceInfo,
        referrer, duration, location, searchQuery 
      } = metadata;
      ...
      logger.error(`Error al registrar vista de perfil: ${error.message}`, { error });
  ...
  async manageProfileImage(profileId, imageData) {
    try {
      const { 
        id, imageUrl, thumbnailUrl, mediumUrl, isMain, 
        description, orderPosition, isPrivate = false 
      } = imageData;
      ...
      const newImage = await prisma.profileImage.create({
        data: {
          ...
          orderPosition,
          isPrivate,
          // Por defecto, nuevas imágenes requieren aprobación
          isApproved: false
        }
      });
      ...
      logger.error(`Error al gestionar imagen de perfil: ${error.message}`, { error });
      throw error;
    }
  }
    ...
    try {
      ...
      await prisma.agencyChange.create({
        data: {
          ...
          status: 'pendiente',
          reason: agencyId ? 'Cambio a nueva agencia' : 'Cambio a independiente'
        }
      });
