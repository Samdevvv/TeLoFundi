...
class AgencyService {
  ...
  async getAgencyProfiles(agencyId, options = {}) {
    try {
      ...
      logger.error(`Error al obtener perfiles de agencia: ${error.message}`, { error });
      throw error;
    }
  }
  ...
  async verifyProfile(agencyId, profileId, approved, verificationData = {}) {
    try {
      ...
      const expirationDays = systemSettings?.autoVerificationExpirationDays || 90;
      ...
      await prisma.notification.create({
        data: {
          ...
          content: approved 
            ...
            : `Tu verificación ha sido rechazada. Motivo: ${verificationData.notes || 'No especificado'}`,
  ...
  async addProfileToAgency(agencyId, profileId) {
    try {
      ...
      logger.error(`Error al añadir perfil a agencia: ${error.message}`, { error });
      throw error;
    }
  }
    ...
    try {
      ...
      await prisma.notification.create({
        data: {
          ...
          content: `Has sido removido de la agencia. Motivo: ${reason || 'No especificado'}`,
          send_email: true
        }
      });
